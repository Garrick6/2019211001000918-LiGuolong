...
import com.LiGuolong.model.Item;
import com.LiGuolong.model.Order;
...
public class OrderDao implements IOrderDao {
	...
	public int save(Connection con, Order order) throws SQLException {
		...
		try {
			...
			String sql = "INSERT INTO userdb.[dbo].[order](CustomerId,PaymentID,OrderDate,FirstName,LastName,Address1,Address2,city,state,PostalCode,Country,Phone,Notes,OrderTotal) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			String lastId = "SELECT max(Order) as orderId from userdb.[dbo].[order] ";//"SELECT max(orderid) as orderId from userdb.order"; for mysql
			...
			String sql1 = "INSERT INTO userdb.dbo.OrderDetail(OrderID,ProductID,price,Quantity,Total) values(?,?,?,?,?)";
	...
	public List<Order> findByProperty(Connection con, String propertyName, Object value) {
		...
		try {
			String queryString = "select * from userdb.[dbo].[order] as model where model." + propertyName + "= ?";//use userdb.Order for mysql
	...
	public List<Order> findAll(Connection con) {
		...
		List<Order> orderList = new ArrayList<Order>();
		try {
			String queryString = "select * from userdb.[dbo].[Order]";// userdb.Order for mysql
			PreparedStatement st = con.prepareStatement(queryString);
			//st.setObject(1, value);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
	...
	public List<Item> findItemsByOrderId(Connection con, int orderId) {
		...
		try {
			String sql = "SELECT 	* FROM userdb.dbo.OrderDetail AS o INNER JOIN userdb.dbo.Product AS p ON o.ProductId=p.ProductId WHERE o.OrderID=" + orderId;
